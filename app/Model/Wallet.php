...
class Wallet extends Model
{
    ...
    static function base58check_en($address)
    {
     $hash0 = hash("sha256", $address);
     $hash1 = hash("sha256", hex2bin($hash0));
     $checksum = substr($hash1, 0, 8);
     $address = $address.hex2bin($checksum);
     $base58add = Wallet::base58_encode($address);
     return $base58add;
    }
    //decode address from base58check string to byte[]
    static function base58check_de($base58add)
    {
     $address = Wallet::base58_decode($base58add);
     $size = strlen($address);
     if ($size != 25) {
        return false;
     }
    ...
    static function hexString2Base58check($hexString){
        $address = hex2bin($hexString);
        $base58add = Wallet::base58check_en($address);
        return $base58add;
    }
    static function base58check2HexString($base58add){
        $address = Wallet::base58check_de($base58add);
        $hexString = bin2hex($address);
        return $hexString;
    }
    static function hexString2Base64($hexString){
        $address = hex2bin($hexString);
        $base64 = base64_encode($address);
        return $base64;
    }
    static function base642HexString($base64){
        $address = base64_decode($base64);
        $hexString = bin2hex($address);
        return $hexString;
    }
    static function base58check2Base64($base58add){
        $address = Wallet::base58check_de($base58add);
        $base64 = base64_encode($address);
        return $base64;
    }
    static function base642Base58check($base64){
        $address = base64_decode($base64);
        $base58add = Wallet::base58check_en($address);
        return $base58add;
    }
